module httpCalls
    ...
    function doHTTPCall(apiEndpoint::String; queryParams::Vector{Pair{String, String}} = Vector{Pair{String, String}}(), bodyParams::Dict{String, Union{Int64, String, Bool}} = DictDict{String, Union{Int64, String, Bool}}(), bearerToken::String = "")
        ...
        result = endpointHTTPMethod == "GET"    ? HTTP.request("GET", string(uri), headers)                      :
                 endpointHTTPMethod == "PUT"    ? HTTP.request("PUT", string(uri), headers, body = bodyParams)   :
                 endpointHTTPMethod == "POST"   ? HTTP.request("POST", string(uri), headers, body = bodyParams)  :
                 endpointHTTPMethod == "PATCH"  ? HTTP.request("PATCH", string(uri), headers, body = bodyParams) :
                 endpointHTTPMethod == "DELETE" ? HTTP.request("DELETE", string(uri), headers)                   : 
                 Nothing; ## -- Should never make it to this option -- ##;
